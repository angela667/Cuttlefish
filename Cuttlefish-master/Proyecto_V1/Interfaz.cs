...
namespace Proyecto_V1
{
    public partial class Interfaz : Form
    {
                ...
                switch (codigo)
                {
                    ...
                    case 5: //INVITAR A OTRO USUARIO
                        ...
                        Invoke(new Action(() =>
                        {
                            if (result == DialogResult.Yes)
                            {
                                ...
                                numPartida = random.Next();
                                byte[] msg = System.Text.Encoding.ASCII.GetBytes("6/" + trozos[1] + "/" + trozos[2] + "/SI/" + numPartida + "/");
                                server.Send(msg);
                    ...
                    case 6: //RESPUESTA A LA INVITACIÓN
                        ...
                        if (trozos[1] == "YES")
                        {
                            ...
                            numPartida = Convert.ToInt32(trozos[2]);
                            MessageBox.Show("Ha aceptado la invitación con numero de partida: " + numPartida);
                    ...
                    case 8: //MOVIMIENTO DEL RIVAL
                        ...
                        string amigo = trozos[2];
                        int mov_rival = Convert.ToInt32(trozos[3]);

                        if(amigo == username)
                        {
                            if (IDpartida == numPartida)
                            {
                                switch (mov_rival)
                                {
                                    case 1: //PIEDRA RIVAL
        ...
        {
            MessageBox.Show("amigo: " + nombre_invitado + "anfitrion: " + username);


            string mensaje = "5/" + nombre_invitado + "/" + username + "/";
        ...
        private void textoChat_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                ...
                if (texto != "")
                {
                    ...
                    textoChat.Text = "";
                    string mensaje = "7/" + mensajeChat + "/" + username + "/";
                    byte[] msg = System.Text.Encoding.ASCII.GetBytes(mensaje);
                    server.Send(msg);
                }
            }
        ...
        {
            string mensaje = "8/" + numPartida + "/" + username + "/1/";
            byte[] msg = System.Text.Encoding.ASCII.GetBytes(mensaje);
            server.Send(msg);
            mi_movimiento = 1;
        }
        ...
        {
            string mensaje = "8/" + numPartida + "/" + username + "/2/";
            byte[] msg = System.Text.Encoding.ASCII.GetBytes(mensaje);
            server.Send(msg);
            mi_movimiento = 2;
        }
        ...
        {
            string mensaje = "8/" + numPartida + "/" + username + "/3/";
            byte[] msg = System.Text.Encoding.ASCII.GetBytes(mensaje);
            server.Send(msg);
            mi_movimiento = 3;
        }

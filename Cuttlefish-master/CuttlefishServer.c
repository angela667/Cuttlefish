...

				{
					...
					if (row != 0){
						...
						if (ret != 0) {
							...
							sprintf (respuesta,"1/Registrado correctamente/%s/\n", Usuario);						
				...
				if (ret != 0) {
					...
					if (row == NULL) {
						...
						if (strcmp(row[0], Contrasena) == 0) {
							...
							int sockk=DameSocket(&miLista,Usuario);

							printf("Sesion iniciada correctamente con socket: %d\n", sock_conn);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			printf("El anfitrion es %s y el invitado es %s \n", anfitrion, invitado);
			...
			int sock_amigo=DameSocket(&miLista,invitado);

			

			sprintf(respuesta,"5/%s/%s/",anfitrion, invitado);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...

		{
			...
			printf("el socket del anfitrion es: %d y el socket del invitado es: %d\n",sockk, sock_amigo);
...

			{
				...
				int anadir_amigo = PonJugadorPartida(&partidA,invitado,sock_amigo);

				

				sprintf(respuesta,"6/YES/%d/",IDpartida);

				printf("Que empiece la partida %d entre %s %s\n", IDpartida, anfitrion, invitado);

				write (sock_amigo,respuesta, strlen(respuesta));

				write (sockk,respuesta, strlen(respuesta));
...

		{
			...
			char amigo[20];
            		...
            		if (p != NULL) {
                		...
                		sprintf(respuesta, "7/%s/%s/", mensaje, amigo);
...

		{
			...
			switch (movimiento) {
				...
				case 1: 
					...
					sprintf(respuesta, "8/%d/%s/1/", IDpartida, amigo);

				break;

				case 2:

					sprintf(respuesta, "8/%d/%s/2/", IDpartida, amigo);

				break;

				case 3:

					sprintf(respuesta, "8/%d/%s/3/", IDpartida, amigo);
